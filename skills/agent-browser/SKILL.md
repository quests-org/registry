...
# The path in `download <sel> <path>` must match where the file will actually be saved.
#
...
agent-browser --download-path ./output open https://example.com/file.docx  # Triggers download directly
#
...
Always set `--download-path` before any download interaction. The path in `download <sel> <path>` must match where the browser will save the file.

> **Note:** Only use `download` on an element that directly initiates a file transfer. If clicking the element navigates to another page, you need to navigate there first, re-snapshot, and target the actual download trigger.

```bash
# Option A: Click a download link/button on the page
agent-browser --download-path ./output open https://example.com/downloads
agent-browser snapshot -i
agent-browser download @e5 ./output/file.docx    # Click element; wait for file at that path

# Option B: Open the file URL directly (most reliable when you know the URL)
agent-browser --download-path ./output open https://example.com/file.docx
# File lands in ./output automatically — no click needed

# Option C: Extract the href first, then open it
agent-browser get attr @e5 href                  # Get the download URL from the link
agent-browser --download-path ./output open <that-url>
```

> **Note:** `--download-path` applies to the whole session. If the browser is already open without it, close and reopen with the flag set, or use `agent-browser wait --download` to catch wherever the file lands.
...
agent-browser find first "button" click      # First matching element
agent-browser find last "li" click           # Last matching element
agent-browser find nth "tr" 2 click          # Nth matching element (0-indexed)
```
