...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!filePath || !values.format) {
    console.error(
      "Usage: tsx skills/images/scripts/convert.ts <path> --format <fmt> [--quality <1-100>] [--output <path>]",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Converted → ${relOutput} (${result.format}, ${result.width}×${result.height}, ${result.bytes} bytes)`,
  );
}
